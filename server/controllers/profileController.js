...
import { MUTUAL_LIKE_SQL, isProfileComplete } from '../utils/queryHelpers.js';
...
export const updateProfile = async (req, res) => {
  ...
  try {
    ...
    if (isProfileComplete(user, hasImages.rows.length > 0, hasTags.rows.length > 0) && !user.profile_complete) {
...
export const updateProfileTags = async (req, res) => {
  ...
  try {
    ...
    if (isProfileComplete(user, hasImages.rows.length > 0, tags.length > 0) && !user.profile_complete) {
